...
const ServiceCard = ({
  ...
  // Стиль карточки зависит от наличия изображения
  return image ? (
    <div className="service-card bg-card rounded-lg shadow-md hover:shadow-xl dark:shadow-white/10 overflow-hidden transition-all duration-200 flex flex-col h-full transform hover:-translate-y-1">
      ...
      <div className="p-3">
        ...
        <Button 
          variant="outline"
          className="w-full mt-3 border-primary text-primary hover:bg-primary hover:text-white shadow-sm hover:shadow-md transition-shadow"
          onClick={() => onClick(id)}
        >
          Подробнее
        </Button>
